...
@telebot.on(admin_cmd(sudo_cmd(pattern="savewelcome")))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@telebot.on(admin_cmd(sudo_cmd(pattern="clearwelcome")))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@telebot.on(admin_cmd(sudo_cmd(pattern="listwelcome")))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
